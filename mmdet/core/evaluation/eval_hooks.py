...
class EvalHookRecon(Hook):
    ...
    def after_train_epoch(self, runner):
        if not self.evaluation_flag(runner):
            return
        from mmdet.apis import single_gpu_test_recon
        epoch = runner.epoch + 1
        out_dir = '{}/epoch_{}_vis'.format(runner.work_dir, epoch)
        single_gpu_test_recon(runner.model, self.dataloader, out_dir=out_dir)
        self.logger.info(
                    f'Evaluation of epoch_{runner.epoch + 1}.pth done')